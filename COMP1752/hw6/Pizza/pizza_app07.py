...
def empty_basket():
    # clear
    basket.clear()
    # clear basket text box
    txt2['state'] = 'normal' # change state from normal to edit
    txt2.delete("1.0", tk.END) # clear content of Entry
    txt2["state"] = 'disabled' # chage state back to disabled to prevent
    # clear total cost
    basket_output.set('')
    txt2['state'] = 'normal'
    txt2.delete("1.0", tk.END)
...
#2
empty_button = tk.Button(window, text="Empty Basket", command=empty_basket)
empty_button.grid(column=1, row=row_counter, sticky="W", padx=5, pady=5)
row_counter += 1
