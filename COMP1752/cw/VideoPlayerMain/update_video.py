...
# For the last one, after the show_current_video_list information function above clicked, the data will appear in the textarea
        self.video_txt = tk.Text(window, width=40, height=10, wrap="none", background="#FF00FF")
        self.video_txt.pack()
...
# The primary function for the operation works
    def update_video_rating(self):
        ...
        if not video_number.isdigit():
            self.show_error_message("Invalid video number!") # If video number is not an integer
            return
        ...
        if not rating.isdigit():
            self.show_error_message("Invalid rating!") # If rating case not an integer
            return
